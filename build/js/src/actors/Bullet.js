...
/** @type {[import('../engine/Actor.js').Actor & {bouncesLeft: number, velocity: THREE.Vector3}]} */
const bullets = [];

export const spawnBullet = (scene, position, angle) => {
  const bullet = { bouncesLeft: 1 };
  const speed = 0.2;
  bullet.mesh = new THREE.Mesh(g_bullet, m_bullet);
  Object.assign(bullet.mesh.position, position);

  bullet.mesh.rotation.y = Math.PI - angle;
...
export const updateBullets = (delta) => {
  const mines = getMines().map((mine) => ({
    mine,
    bounding: mine.getBoundingBox(),
  }));

  for (let i = 0; i < bullets.length; ++i) {
    const bullet = bullets[i];
    bullet.mesh.position.add(
      bullet.velocity.clone().multiplyScalar(delta * 30)
    );
    const bounding = bullet.getBoundingBox();

    for (let i = 0; i < mines.length; ++i) {
      const mine = mines[i];
      if (mine.bounding.intersectsBox(bounding)) {
        bullet.destroy();
        mine.mine.destroy();
      }
    }
